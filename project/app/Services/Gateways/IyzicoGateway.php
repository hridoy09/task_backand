...
class IyzicoGateway extends Gateway implements PaymentGatewayInterface
{
    ...
    public function create(Payment $payment): string
    {
        ...
        $iyzicoCurrency = defined(Currency::class . '::' . $currencyCode)
            ? constant(Currency::class . '::' . $currencyCode)
            : Currency::USD;

        $requestIyzico = new CreateCheckoutFormInitializeRequest();
        $requestIyzico->setLocale(Locale::EN);
        $requestIyzico->setPrice(number_format($charge['amount'], 2, '.', ''));
        $requestIyzico->setPaidPrice(number_format($charge['amount'], 2, '.', ''));
        $requestIyzico->setCurrency($iyzicoCurrency);
        ...
        $basketItem->setPrice($charge['amount']);
